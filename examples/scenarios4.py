...
departure = og.distance.nearest_nodes(street_graph, 10.845161,45.891972) #bus station riva del garda
street_graph = og.routing.add_time_to_edge(street_graph)
tracks = og.routing.track_from_node(street_graph, departure, 3000, length_margin= 1000)
for i,t in enumerate(tracks):
    ngdf, egdf = og.routing.route_to_gdf(street_graph, tracks[i][1], nodes=True) # type: ignore
    l = egdf["length"].sum()
    print(f"Track {i} has a length of {l} meters")

# subset = [og.routing.simplyfied_path_gdf(og.routing.route_to_gdf(street_graph, list_tr)) for _, list_tr in tracks]
