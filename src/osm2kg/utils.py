...
def cut_gdf_on_geometry(gdf : gpd.GeoDataFrame, geometry: Polygon | MultiPolygon) -> gpd.GeoDataFrame:
    """
    This function maintains only the intersection of the gdf with the geometry.
    Woring like a cut and preserving only the part of the gdf that is inside the geometry.
    If the intersection pruduces a GeometryCollection, it will be converted to a MultiPolygon,
    excluding all the geometry that are not Polygon or MultiPolygon.
    ...
    geometry : shapely.geometry
        The geometry to cut the gdf with.
    
    Returns
    -------
    gpd.GeoDataFrame
        The gdf cutted with the geometry.
    """
    if gdf.empty:
        return gdf
    if geometry.is_empty:
        return gdf
    gdf = gdf.copy()
    gdf["geometry"] = gdf["geometry"].intersection(geometry)
    index = gdf[gdf.geometry.geom_type == "GeometryCollection"].index
    if len(index) > 0:
        gdf.loc[index, "geometry"] = gdf.loc[index, "geometry"].apply(lambda x: MultiPolygon([ g for g in x.geoms if (isinstance(g, Polygon) or isinstance(g, MultiPolygon))]))
