"""
Module for calculating distances and finding nearest nodes/edges in a graph.
"""
from __future__ import annotations
from typing import overload, Optional, Dict, Union
...
# if X and Y are floats and return_dist is not provided (defaults False)
@overload
def nearest_edges(gdf: gpd.GeoDataFrame, X: float, Y: float, *, including_filter: Optional[Dict[str, Union[str, bool, list]]] = None, excluding_filter: Optional[Dict[str, Union[str, bool, list]]] = None,) -> tuple[int, int, int]: ...
...
def nearest_edges(
    ...
    *,
    return_dist: Literal[False],
    including_filter: Optional[Dict[str, Union[str, bool, list]]] = None,
    excluding_filter: Optional[Dict[str, Union[str, bool, list]]] = None,
) -> tuple[int, int, int]: ...
...
def nearest_edges(
    ...
    *,
    ...
    use a projected gdf and projected points.
    You can also filter the nearest edge that will be found.
    Tree formats are supported:
    ...
    ----------
    gdf
        GeoDataFrame containing the edges to search within.
    X
        The points' x (longitude) coordinates, in same CRS/units as gdf and
        containing no nulls.
    Y
        The points' y (latitude) coordinates, in same CRS/units as gdf and
        containing no nulls.
    return_dist
        If True, optionally also return the distance(s) between point(s) and
        nearest edge(s), in same units as gdf and points.
    including_filter
        A dictionary of filters to include edges in the search.
    excluding_filter
        A dictionary of filters to exclude edges in the search.

    Returns
    -------
    ne or (ne, dist)
        Nearest edge ID(s) from the GeoDataFrame index, or optionally a tuple of
    ...
    if filter is not None:
        geoms = feature.filter_gdf(gdf, including_filters=including_filter, excluding_filters=excluding_filter)["geometry"]
    else:
        geoms = gdf["geometry"]
