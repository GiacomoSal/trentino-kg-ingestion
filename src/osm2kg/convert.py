...
def graph_to_gdfs(
    ...
    *,
    ...
    if nodes:
        ...
        gdf_nodes.index = gdf_nodes.index.rename(["element","osmid"])
    ...
    *,
    ...
    if nodes:
        ...
        if node_geometry:
            # convert node x/y attributes to Points for geometry column
            node_geoms = (Point(d["x"], d["y"]) for d in data)
            # gdf_nodes = gpd.GeoDataFrame(data, index=uvk, crs=crs, geometry=list(node_geoms))  to do : restore
            gdf_nodes = gpd.GeoDataFrame(data, index=uvk, crs=crs, geometry=list(node_geoms))            
        else:
            gdf_nodes = gpd.GeoDataFrame(data, index=uvk)
...
def _validate_node_edge_gdfs_digraph(
    gdf_nodes: gpd.GeoDataFrame,
    gdf_edges: gpd.GeoDataFrame,
) -> None:
    """
...
def digraph_from_gdfs(
    ...
    *,
    ...
    G
        The converted MultiDiGraph.
    """
    _validate_node_edge_gdfs_digraph(gdf_nodes, gdf_edges)
    ...
    msg = "Created digraph from node/edge GeoDataFrames"
    utils.log(msg, level=lg.INFO)
    return G
