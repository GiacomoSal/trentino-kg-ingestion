...
def features_from_bbox(
    ...
    special_filter
        If True, apply a special filter to the geometries that intersect the
        bounding box but are not fully contained within it. This is useful for
        filtering out features that only partially intersect the bounding box.
    ...
    return features_from_polygon(polygon, tags, special_filter=special_filter)
...
def features_from_place(
    ...
    *,
    ...
    special_filter
        If True, apply a special filter to the geometries that intersect the
        polygon but are not fully contained within it. This is useful for
        filtering out features that only partially intersect the polygon.
...
def features_from_polygon(
    ...
    response_jsons = ox._overpass._download_overpass_features(polygon, tags)
    return _create_gdf(response_jsons, polygon, tags, special_filter=special_filter)
...
def features_from_xml(
    ...
    *,
    ...
    gdf = _create_gdf(response_jsons, polygon, tags, special_filter=special_filter)
...
def _create_gdf(
    ...
    return _filter_features(gdf, polygon, tags, special_filter=special_filter)
...
def _filter_features(
    ...
    if polygon.is_empty:
        ...
        if special_filter:
            intersects = gdf.geometry.intersects(polygon) # bool array with intersections
            # bool array with contained geometries
            within = gdf.geometry.within(polygon)
            # subset those that intersect but are not within
            intersect_but_not_within = intersects & ~within
            subset = gdf[intersect_but_not_within]
            # now filter on geometries with intersect area less than 1% (0.01)
            warnings.filterwarnings("ignore", message=".*CRS.*")
            intersection_area = subset.geometry.intersection(polygon).area
            original_area = subset.geometry.area
            warnings.filterwarnings("default", message=".*CRS.*")
            ratio = intersection_area / original_area
            keep_mask = (ratio > 0.01) | (ratio == 0)
            # print(f"keep_mask: {keep_mask}")
            indices_to_keep = subset.index[keep_mask]

            geom_filter = within | gdf.index.isin(indices_to_keep)
        else:
            # ---
            idx = ox.utils_geo._intersect_index_quadrats(gdf["geometry"], polygon)
...
# --------------------------------------------------------------------------------
def extract_nodes_from_feature_gdf(
    gdf: gpd.GeoDataFrame,
) -> set:
    """
    Extract nodes, used for graph search from a GeoDataFrame of OSM features.
    ATTENTION: This function extracts nodes id from the GeoDataFrame, you need to
    add the feature in the GeoDataFrameto the graph using the 'graph.add_feature_on_graph()' function
    to use the resulting set in meaningful way.

    Parameters
    ----------
    gdf
        The GeoDataFrame of OSM features.

    Returns
    -------
    set
        A set of node IDs extracted from the GeoDataFrame.
    """
    # filter the GeoDataFrame to retain only nodes
    return set(gdf.index.to_list())


def filter_gdf(
        gdf: gpd.GeoDataFrame,
        including_filters: dict = None, excluding_filters: dict = None,
        include_with_or :bool = True, exclude_with_or: bool = True) -> gpd.GeoDataFrame:
    """
    Filter the gdf based on the filters provided.
    The filters are a dictionary where the key is the tag and the value is the value to filter on.
    The filters can be:
    1. {"tag_name": "tag_value"}: Only include/exclude edges that have this tag value in their columns.
    2. {"tag_name": True}: Only include/exclude edges that have this tag key in their columns.
    3. {"tag_name": ["tag_value1", "tag_value2"]}: Only include/exclude edges that have this tag value in their columns.
    if the filter is passed as **including_filters**, the rows that match the filters will be kept.
    if the filter is passed as **excluding_filters**, the rows that match the filters will be excluded.

    Parameters
    ----------
    gdf : gpd.GeoDataFrame
        The gdf to filter.
    including_filters : dict
        The filters to apply, mantaining only the rows that match the filters.
    excluding_filters : dict
        The filters to apply, excluding the rows that match the filters. 
    include_with_or : bool, optional
        If True, the filters will be applied with an OR operation, meaning that if any of the filters match, the row will be kept.
        If False, the filters will be applied with an AND operation, meaning that all filters must match for the row to be kept.
        The default is True.
    exclude_with_or : bool, optional
        If True, the filters will be applied with an OR operation, meaning that if any of the filters match, the row will be excluded.
        If False, the filters will be applied with an AND operation, meaning that all filters must match for the row to be excluded.
        The default is True.
    ...
    gpd.GeoDataFrame
        The filtered gdf.
    """
    if including_filters==None and excluding_filters==None:
        return gdf

    if include_with_or and including_filters is not None:
        inc_mask = pd.Series(False, index=gdf.index)
    else:
        inc_mask = pd.Series(True, index=gdf.index)
    if including_filters is not None:
        for tag_key, tag_value in including_filters.items():
    ...
    if exclude_with_or or excluding_filters is None:
        esc_mask = pd.Series(False, index=gdf.index)
    else:
        esc_mask = pd.Series(True, index=gdf.index)    
    if excluding_filters is not None:
        for tag_key, tag_value in excluding_filters.items():
            if tag_key not in gdf.columns:
                continue
            ...
            if exclude_with_or:
                ...
                esc_mask &= cond
    mask = inc_mask & ~esc_mask
    return gdf[mask]


def add_entrance_to_feature(
        feature_gdf: gpd.GeoDataFrame,
        entrance_tag={"entrance":True},
        fill_data_from_entrance=True,
        use_centroid_if_None=True
        )-> gpd.GeoDataFrame:
    """
    Add entrance to feature gdf using two column: entrance for the type and entrance_geometry for the 
    geometry of it, than it can be used to add entrance to the graph.
    You culd also use personalized tags to add entrance instead of the default one that search
    for entrance tag on the OSM data.
    
    Parameters
    ----------
    feature_gdf : gpd.GeoDataFrame
        The feature gdf to add entrance to.
    entrance_tag : dict, optional
        The entrance tag to use. The default is {"entrance":True}.
    fill_data_from_entrance : bool, optional
        If True, the data from the entrance will be used to fill the tag with smae name of the feature. The default is True.
    use_centroid_if_None : bool, optional
        If True, the centroid of the feature will be used as entrance if no entrance is found. The default is True.
        If False, the resutl gdf will not have the row
    
    Returns
    -------
    gpd.GeoDataFrame
        The feature gdf with entrance added as two new column (entrance and entrance_geometry).
    """
    features_pol = feature_gdf[feature_gdf["geometry"].type!= "Point"]["geometry"].union_all()
    entrance_gdf = features_from_polygon(features_pol, tags=entrance_tag)
    entrance_gdf["entrance_geometry"] = entrance_gdf["geometry"]
    gdf = gpd.sjoin(feature_gdf, entrance_gdf, how="left", predicate='intersects', lsuffix=None)
    gdf.drop(["element_right", "id_right"], axis=1, inplace=True)
    gdf.index.names = ['element', 'id']

    if fill_data_from_entrance:
        column_right = [col for col in gdf.columns if col.endswith('_right')]
        coloumn = set(col.replace('_right', '') for col in column_right)
        for col, col_r in zip(coloumn, column_right):
            gdf[col] = gdf[col].fillna(gdf[col_r])
        gdf = gdf.drop(columns=[col for col in column_right])

    if use_centroid_if_None:
        geom_series = gdf["geometry"]
        to_crs = geom_series.estimate_utm_crs()
        geom_series = geom_series.to_crs(crs=to_crs)
        gdf["entrance_geometry"] = gdf["entrance_geometry"].fillna(geom_series.centroid.to_crs(crs="EPSG:4326"))
    return gdf